...
public class LocalRBTree<K extends Comparable<K>, V> {
    ...
    private void insertFixedUp() {
         ...
         *       5- 插入点的父点为红色 且叔点不存在或者为黑色 父点为爷爷点的左子 且当前点为父点的左子 LL情况    ===>   父变黑，爷变红，爷点右旋即可
         *       6- 插入点的父点为红色 且叔点不存在或者为黑色 父点为爷爷点的左子 且当前点为父点的右子 LR情况    ===>   以父点左旋一次，得到LL双红，然后走上面
         *
         *       7- 插入点的父点为红色 且叔点不存在或者为黑色 父点为爷爷点的右子 且当前点为父点的左子 RL情况    ===>   以父点右旋一次，得到RR双红，然后走下面
         *       8- 插入点的父点为红色 且叔点不存在或者为黑色 父点为爷爷点的右子 且当前点为父点的右子 RR情况    ===>   父变黑，爷变红，爷点左旋即可
         *
         *
         *
         *
